...
interface Props {
  onUpdate: (
    ...
    expiracaoCount: number,
    manutencoesProximasLength: number
  ) => void;
  className?: string;
}
  ...
  buscarDadosAtualizados = () => {
    ...
    Promise.all([
      ...
      .then(([ativosTangiveisData, ativosIntangiveisData, manutencoesData]) => {
        if (
          !Array.isArray(ativosTangiveisData) ||
          !Array.isArray(ativosIntangiveisData) ||
          !Array.isArray(manutencoesData)
        ) {
        ...
        const ativosExpiracao = ativosIntangiveisData
          .filter((ativo: any) => {
            const dataExpiracao = moment(ativo.dataExpiracao);
            const today = moment();
            return (
              dataExpiracao.isValid() && dataExpiracao.diff(today, "days") <= 10
            );
          })
          .map((ativo: any) => ({
            id: ativo.id,
        ...
        const manutencoesProximas = manutencoesData
          .filter((manutencao: any) => {
            const dataFim = moment(manutencao.dataFim);
            const today = moment();
            return (
              dataFim.isValid() &&
              dataFim.isAfter(today) &&
              dataFim.diff(today, "days") <= 10
            );
          })
          .map((manutencao: any) => ({
            id: manutencao.id,
        ...
        this.setState(
          ...
          () => {
            this.props.onUpdate(
              this.state.ativosGarantia.length,
              this.state.ativosExpiracao.length,
              this.state.manutencoesProximas.length
            );
          }
        );
      })
                ...
                return (
                  <li key={index}>
                    {diffDays === 0
                      ...
                      ? `A garantia do ativo ${
                          ativo.id
                        } terminou em ${dataGarantia.format("DD/MM/YYYY")}`
                ...
                return (
                  <li key={index}>
                    {diffDays === 0
                      ...
                      ? `A licença do ativo ${
                          ativo.id
                        } expirou em ${dataExpiracao.format("DD/MM/YYYY")}`
