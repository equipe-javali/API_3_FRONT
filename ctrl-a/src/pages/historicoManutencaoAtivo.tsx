...
export default function HistoricoManutencao() {
  ...
  function LinhaManutencao({
    ...
    function aparecerBotaoDeMudarDataRetorno() {
      if (!dataFim) {
        ...
        return <>{moment(dataFim).format("DD/MM/YYYY")}</>;
      }
    }
    ...
    return (
        ...
        >
          ...
          <td className="custo">{formattedCusto}</td> 
  ...
  function TabelaManutencao({
    manutencao,
    filtro,
  }: TabelaManutencaoProps & { filtro: string }) {
    const linhas = manutencao
      .filter((man) =>
        filtro ? reverseTipoMapping[Number(man.tipo)] === filtro : true
      )
  ...
  const [showManutencaoModal, setShowManutencaoModal] =
    useState<boolean>(false);
  const [showManutencaoAtualizacaoModal, setShowManutencaoAtualizacaoModal] =
    useState<boolean>(false);
  ...
  const Pesquisando = sortedManutencao.filter((manutencao) => {
    ...
    if (Pesquisa === "" || Pesquisa === "Todos") {
      return Object.values(manutencao).some(
        (value) =>
          typeof value === "string" &&
      ...
      return (
        reverseTipoMapping[Number(manutencao.tipo)] === Pesquisa &&
        Object.values(manutencao).some(
          (value) =>
            typeof value === "string" &&
            value.toLowerCase().includes(searchTermLower)
        )
  ...
  function applyCustoMask(valorOriginal: string): string {
    if (valorOriginal.trim() !== '') {       
      const valorNumerico = parseFloat(valorOriginal.replace(/\D/g, '').replace(',', '.')) / 100;
      const partes = valorNumerico.toFixed(2).split('.'); 
      let reais = partes[0];
      let centavos = "00"; 
      if (partes.length > 1) { 
        centavos = partes[1].slice(0, 2); 
      }
      reais = reais.replace(/\B(?=(\d{3})+(?!\d))/g, ","); 
      return `R$ ${reais.replace('.', ',')},${centavos}`; 
    } else {
      return 'R$ 0,00'; 
    }
  }
  const handleManutencaoDataChange = (
    ...
    if (name === "custo") {
      const formattedValue = applyCustoMask(value);
      setManutencaoData((prevData) => ({
        ...prevData,
  ...
  function handleTextareaDataChange(
  ...
  function handleManutencaoSubmit() {
    ...
    const manutencaoDataWithDates = {
      ...manutencaoData,
      tipo:
        typeof manutencaoData.tipo === "string"
          ? tipoMapping[manutencaoData.tipo] || 0
          : manutencaoData.tipo,
      dataInicio: manutencaoData.dataInicio
        ? new Date(manutencaoData.dataInicio).toISOString()
        : currentDate,
      dataFim: manutencaoData.dataFim
        ? new Date(manutencaoData.dataFim).toISOString()
        : null,
      ativo: { id: manutencaoData.ativoId },
    };

    
    const custoFormatado = applyCustoMask(manutencaoData.custo);
  const custoToDecimal = parseFloat(custoFormatado.replace(/\D/g, "").replace(",", ".")) / 100;
  ...
  return (
    <div className="dashboardMan">
      <div className="tituloMan tituloBotao">
        ...
        <button className="btnManutencao" onClick={toggleModal}>
          Adicionar pedido de manutenção
        </button>
      </div>
      <Modal
        open={showManutencaoModal}
        onClose={handleManutencaoSubmit}
        ...
        <div>
          <div className="containerModal">
            <div className="modal-man">
              ...
              <input
                name="localizacao"
                value={manutencaoData.localizacao}
          ...
          <div className="containerModal">
            <div className="modal-man">
              ...
              <input
                type="date"
                name="dataInicio"
                value={manutencaoData.dataInicio}
                onChange={handleManutencaoDataChange}
              />
            </div>
            <div className="modal-man">
          ...
          <div className="containerModal">
            ...
            <div className="modal-man">
              ...
              <select
                name="tipo"
                value={reverseTipoMapping[Number(manutencaoData.tipo)]}
      ...
      <Modal
        open={showManutencaoAtualizacaoModal}
        onClose={handleAtualizacaoAtualizar}
        onCancel={handleCancelAtualizacao}
        title="Mudança de data de retorno"
      >
        <div>
          <div className="containerModal">
            <div className="modal-man">
